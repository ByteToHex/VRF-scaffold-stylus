 ...
 * This script is used to deploy Solidity contracts using Foundry
 */
if (require.main === module) {
  // Use yargs for argument parsing
  const argv = yargs(hideBin(process.argv))
    .usage("Usage: yarn solidity:deploy --network <network>")
    .option("network", {
      alias: "net",
    ...
    .option("verify", {
      alias: "v",
      describe: "Verify contract on block explorer",
      type: "boolean",
      demandOption: false,
    })
    .help()
